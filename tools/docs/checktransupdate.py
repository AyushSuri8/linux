...
1 commit needs resolving in total
"""
...
HASH_PATTERN = r"([0-9a-f]{12,40})"
PAT_UPDATE_TO = re.compile(rf"update to commit {HASH_PATTERN}", re.IGNORECASE)
PAT_UPDATE_TRANSLATION = re.compile(
    rf"update the translation through commit {HASH_PATTERN}", re.IGNORECASE
)


def run_git_command(args):
    """Run a git command and return stdout; log and return None on error."""
    cmd = ["git"] + args
    ...
    try:
        ...
        logging.error("Failed to run command '%s': %s", " ".join(cmd), err)
        return None
...
def get_origin_path(file_path):
    ...
    # Expected structure:
    # Documentation/translations/<locale>/...
    if "translations" not in paths:
        return None

    tidx = paths.index("translations")
    if tidx == 0 or tidx + 2 >= len(paths):
        return None
...
def get_latest_commit_from(file_path, commit):
    ...
    result = run_git_command(
        ["log", "--pretty=format:%H%n%aD%n%cD%n%n%B", commit, "-1", "--", file_path]
    )
    if not result:
        return None
    ...
    logging.debug("Result: %s", lines[0])

    try:
        return {
            "hash": lines[0],
            "author_date": datetime.strptime(lines[1], "%a, %d %b %Y %H:%M:%S %z"),
            "commit_date": datetime.strptime(lines[2], "%a, %d %b %Y %H:%M:%S %z"),
            "message": lines[4:],
        }
    except (ValueError, IndexError) as err:
        logging.error("Failed to parse git log output for %s: %s", file_path, err)
        return None
...
def get_origin_from_trans_smartly(origin_path, t_from_head):
    """Get the latest origin commit from formatted translation commit messages."""
...
def get_commits_count_between(opath, commit1, commit2):
    """Get the commit hashes between two commits for the specified file."""
    result = run_git_command(["log", "--pretty=format:%H", f"{commit1}...{commit2}", "--", opath])
    if result is None:
        return []
...
def pretty_output(commit):
    ...
    result = run_git_command(["log", '--pretty=format:%h ("%s")', "-1", commit])
    if result is None:
        return ""
    return result
...
def valid_commit(commit):
    """Check if the commit should be counted as requiring translation work."""
...
def check_per_file(file_path):
    ...
    if opath is None:
        logging.error("Invalid translation path: %s", file_path)
        return
    ...
    commits = get_commits_count_between(opath, o_from_t["hash"], o_from_head["hash"])
    count = 0
    ...
    logging.info("%d %s need resolving in total\n", count, noun)
...
def valid_locales(locale):
    ...
    locale_path = os.path.join(linux_path, "Documentation", "translations", locale)
...
def list_files_with_excluding_folders(folder, exclude_folders, include_suffix):
    ...
    while stack:
        ...
        try:
            ...
            logging.warning("Unable to list directory %s: %s", pwd, err)
            continue
        ...
        for item in entries:
            ...
            elif ab_item.endswith(include_suffix):
...
def config_logging(log_level, log_file="checktransupdate.log"):
    ...
    # Avoid duplicated logs when reconfigured in embedded usage.
...
def main():
    ...
    # Configure logging based on the --log and --logfile arguments.
    ...
    files = args.files
    if len(files) == 0:
        official_files = list_files_with_excluding_folders(
        ...
        for file in official_files:
            ...
            elif args.print_missing_translations:
                logging.info(os.path.relpath(os.path.abspath(file), linux_path))
                logging.info("No translation in the locale of %s\n", args.locale)

    files = [os.path.relpath(os.path.abspath(path), linux_path) for path in files]
